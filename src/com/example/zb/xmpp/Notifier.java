...
public class Notifier {
    ...
    private ChatActivity currentActivity = null;
    
    public Notifier(Context context, ChatActivity currentActivity) {
    ...
    public void notify(String notificationId, String apiKey, String title,
            String message, String uri,String userid,String username) {
        Log.d(LOGTAG, "notify()...");
...
//            intent.putExtra(Constants.NOTIFICATION_URI, uri);
            ...
            notification.setLatestEventInfo(context, "来自路人的求助", message,
                    contentIntent);
            
            if(ChatActivityManager.getCurrentActivity() ==null ||(!ChatActivityManager.aimId.equals(userid)))//如果当前界面不在前台,或者当前的目标用户不是信息来源的用户
            {
//            	ChatActivityManager.getCurrentActivity().finish();
            	notificationManager.notify(random.nextInt(), notification);//通知  
            }
            else                                                 //如果在前台
            	currentActivity.receive(message);         //接收并刷新
